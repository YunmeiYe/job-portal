...
const JobCategory = () => {
  return (
    <div className="mt-20 pb-5">
      ...
      <div className="text-lg sm-mx:text-base xsm-mx:text-sm text-mine-shaft-300 text-center mx-auto mb-10 w-1/2 sm-mx:w-11/12">
      ...
      <Carousel
        ...
        {jobCategories.map((category, index) =>
          <CarouselSlide key={index}>
            <div className="flex flex-col items-center w-64 h-58 sm-mx:w-56 sm-mx:h-52 xs-mx:w-48 xs-mx:h-42 gap-2 border border-bright-sun-400 p-5 my-5 transition duration-300 ease-in-out rounded-xl hover:cursor-pointer hover:shadow-[0_0_5px_2px_black] !shadow-bright-sun-300">
              ...
              <div className="text-mine-shaft-200 text-xl sm-mx:text-lg xs-mx:text-base font-semibold">{category.name}</div>
              <div className="text-sm xs-mx:text-xs text-center text-mine-shaft-300">{category.desc}</div>
              <div className="text-lg sm-mx:text-base xs-mx:text-sm text-bright-sun-300">{category.jobs} new job posted</div>
            </div>
          </CarouselSlide>)}
      </Carousel>
    </div>
