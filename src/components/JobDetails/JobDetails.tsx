...
const JobDetails = (props: any) => {
  ...
  return (
    <div className="w-2/3 mx-auto bs-mx:w-full">
      <div className="flex flex-wrap justify-between items-center gap-2">
        <div className="flex gap-2 items-center">
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="text-sm text-mine-shaft-300 flex flex-wrap xs-mx:text-xs">
        ...
        <div className="flex sm:flex-col gap-2 items-center sm-mx:my-2 sm-mx:w-full sm-mx:[&>button]:w-1/2 sm-mx:[&>a]:w-1/2">
          {(props.edit || !applied) &&
            <Link to={props.edit ? `/post-job/${props.id}` : `/apply-job/${props.id}`} className="text-center">
              <Button color="brightSun.4" size="sm" fullWidth variant="light" >{props.closed ? "Reopen" : props.edit ? "Edit" : "Apply"}</Button>
            </Link>
          }
      ...
      <div className="flex sm-mx:flex-wrap justify-between gap-4 sm-mx:gap-2">
        {card.map((item, index) =>
          <div key={index} className="flex flex-col items-center gap-1">
            <ActionIcon className="!h-12 !w-12 xs-mx:!h-8 xs-mx:!w-8" color="brightSun.4" variant="light" size="lg" radius="xl" aria-label="Settings">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {props?.skillsRequired?.map((skill: string, index: number) =>
            <ActionIcon key={index} className="!h-fit !w-fit font-medium !text-sm xs-mx:!text-xs" color="brightSun.4" variant="light" size="lg" p="xs" radius="xl" aria-label="Settings">
              {skill}
            </ActionIcon>
          )}
        </div>
      ...
      <Divider my="xl" />
      <div className="[&_*]:text-mine-shaft-300 [&_li]:marker:text-bright-sun-400 [&_li]:mb-1 [&_h4]:text-xl [&_h4]:my-5 [&_h4]:font-semibold [&_h4]:text-mine-shaft-200 [&_p]:text-justify [&_p]:text-sm [&_li]:text-sm" dangerouslySetInnerHTML={{ __html: cleanHTML }}>
      </div>
      <Divider my="xl" />
      <div>
